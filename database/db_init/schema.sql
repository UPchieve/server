...
CREATE VIEW upchieve.user_session_metrics_view AS
 ...
 SELECT metrics_by_role.user_id,
    COALESCE(sum(metrics_by_role.absent_student), (0)::bigint) AS absent_student,
    COALESCE(sum(metrics_by_role.absent_volunteer), (0)::bigint) AS absent_volunteer,
    COALESCE(sum(metrics_by_role.low_session_rating_from_coach), (0)::bigint) AS low_session_rating_from_coach,
    COALESCE(sum(metrics_by_role.low_session_rating_from_student), (0)::bigint) AS low_session_rating_from_student,
    COALESCE(sum(metrics_by_role.low_coach_rating_from_student), (0)::bigint) AS low_coach_rating_from_student,
    COALESCE(sum(metrics_by_role.reported), (0)::bigint) AS reported,
    COALESCE(sum(metrics_by_role.only_looking_for_answers), (0)::bigint) AS only_looking_for_answers,
    COALESCE(sum(metrics_by_role.rude_or_inappropriate), (0)::bigint) AS rude_or_inappropriate,
    COALESCE(sum(metrics_by_role.comment_from_student), (0)::bigint) AS comment_from_student,
    COALESCE(sum(metrics_by_role.comment_from_volunteer), (0)::bigint) AS comment_from_volunteer,
    COALESCE(sum(metrics_by_role.has_been_unmatched), (0)::bigint) AS has_been_unmatched,
    COALESCE(sum(metrics_by_role.has_had_technical_issues), (0)::bigint) AS has_had_technical_issues,
    COALESCE(sum(metrics_by_role.personal_identifying_info), (0)::bigint) AS personal_identifying_info,
    COALESCE(sum(metrics_by_role.graded_assignment), (0)::bigint) AS graded_assignment,
    COALESCE(sum(metrics_by_role.coach_uncomfortable), (0)::bigint) AS coach_uncomfortable,
    COALESCE(sum(metrics_by_role.student_crisis), (0)::bigint) AS student_crisis,
