...
--
CREATE FUNCTION upchieve.generate_ulid ()
    RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
...
--
CREATE FUNCTION upchieve.get_next_grade_name (current_grade_name text, school_years_passed integer)
    RETURNS text
    LANGUAGE plpgsql
    AS $$
DECLARE
...
--
CREATE FUNCTION upchieve.refresh_users_subjects_mview ()
    RETURNS TRIGGER
    LANGUAGE plpgsql
    AS $$
BEGIN
...
WITH base_dates AS (
    SELECT
        ...
        CASE WHEN (student_profiles.created_at < '2023-08-30'::date) THEN
        ('2023-08-30'::date)::timestamp with time zone
    ELSE
...
grade_progression AS (
    SELECT
        ...
        (
            CASE WHEN (base_dates.base_date >= (date_trunc('year'::text, (CURRENT_DATE)::timestamp with time zone) + '6 mons'::interval)) THEN
            (0)::numeric
        ELSE
            (EXTRACT(year FROM age((CURRENT_DATE)::timestamp with time zone, base_dates.base_date)) + (
                    CASE WHEN (CURRENT_DATE >= (date_trunc('year'::text, (CURRENT_DATE)::timestamp with time zone) + '6 mons'::interval)) THEN
                        1
                    ELSE
                        0
                    END)::numeric)
...
ALTER TABLE upchieve.question_tags
    ...
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.question_tags_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE upchieve.question_types
    ALTER COLUMN id
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.question_types_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
--
ALTER TABLE upchieve.quiz_review_materials
    ALTER COLUMN id
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.quiz_review_materials_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE upchieve.survey_questions
    ALTER COLUMN id
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.survey_questions_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE upchieve.survey_questions_question_tags
    ALTER COLUMN id
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.survey_questions_question_tags_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE upchieve.survey_response_choices
    ALTER COLUMN id
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.survey_response_choices_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE upchieve.survey_types
    ALTER COLUMN id
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.survey_types_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE upchieve.surveys
    ALTER COLUMN id
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.surveys_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE upchieve.surveys_survey_questions
    ALTER COLUMN id
    ADD GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME
        upchieve.surveys_survey_questions_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
flag_rows_by_user AS (
    SELECT
        flags_with_users.student_id AS user_id,
...
UNION ALL
SELECT
    flags_with_users.volunteer_id AS user_id,
...
FROM ((upchieve.users
    ...
    LEFT JOIN (
        ...
        GROUP BY
            users_certifications.user_id, subjects.id, subject_totals.total
        HAVING ((count(*))::integer >= subject_totals.total)) sub_unlocked
GROUP BY
    sub_unlocked.user_id, sub_unlocked.subject) subjects_unlocked ON ((subjects_unlocked.user_id = users.id)))
...
ALTER TABLE ONLY upchieve.notification_priority_groups
    ALTER COLUMN id SET DEFAULT nextval('upchieve.notification_priority_groups_id_seq'::regclass);

--
-- Name: notification_types id; Type: DEFAULT; Schema: upchieve; Owner: -
--
ALTER TABLE ONLY upchieve.notification_types
    ALTER COLUMN id SET DEFAULT nextval('upchieve.notification_types_id_seq'::regclass);

--
-- Name: photo_id_statuses id; Type: DEFAULT; Schema: upchieve; Owner: -
--
ALTER TABLE ONLY upchieve.photo_id_statuses
    ALTER COLUMN id SET DEFAULT nextval('upchieve.photo_id_statuses_id_seq'::regclass);
...
ALTER TABLE ONLY upchieve.progress_report_analysis_types
    ALTER COLUMN id SET DEFAULT nextval('upchieve.progress_report_analysis_types_id_seq'::regclass);
...
ALTER TABLE ONLY upchieve.progress_report_focus_areas
    ALTER COLUMN id SET DEFAULT nextval('upchieve.progress_report_focus_areas_id_seq'::regclass);
...
ALTER TABLE ONLY upchieve.volunteer_reference_statuses
    ALTER COLUMN id SET DEFAULT nextval('upchieve.volunteer_reference_statuses_id_seq'::regclass);

--
-- Name: weekdays id; Type: DEFAULT; Schema: upchieve; Owner: -
--
ALTER TABLE ONLY upchieve.weekdays
    ALTER COLUMN id SET DEFAULT nextval('upchieve.weekdays_id_seq'::regclass);
...
--
CREATE UNIQUE INDEX progress_report_prompt_unique_active_subject_id ON upchieve.progress_report_prompts USING btree (subject_id)
WHERE
    active;
...
--
CREATE UNIQUE INDEX tutor_bot_conversations_unique_non_null_session_id ON upchieve.tutor_bot_conversations USING btree (session_id)
...
--
CREATE TRIGGER update_users_subjects
    AFTER INSERT OR DELETE OR UPDATE ON upchieve.users_certifications
    FOR EACH ROW
    EXECUTE FUNCTION upchieve.refresh_users_subjects_mview ();
...
INSERT INTO public.schema_migrations (version)
    VALUES ('20211026204222'), ('20211026204728'), ('20211026205335'), ('20211026205336'), ('20211026205851'), ('20211026210226'), ('20211026212926'), ('20211026213627'), ('20211026215046'), ('20211026215053'), ('20211026215100'), ('20211026215527'), ('20211026215528'), ('20211026220716'), ('20211026220742'), ('20211026220747'), ('20211026220750'), ('20211026221439'), ('20211109204227'), ('20211109204327'), ('20211109204504'), ('20211109204648'), ('20211109205544'), ('20211109205821'), ('20211109210727'), ('20211109210828'), ('20211109210830'), ('20211109210832'), ('20211109210900'), ('20211109210901'), ('20211109210902'), ('20211109210905'), ('20211109210906'), ('20211109210907'), ('20211109210908'), ('20211109210909'), ('20211109210916'), ('20211109210917'), ('20211109210918'), ('20211109211000'),