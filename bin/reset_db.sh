...
PGPASSFILE='database/.pgpass' dropdb -h "${PGHOST:-localhost}" -p "${PGPORT:-5432}" -U admin -w --if-exists upchieve 1> /dev/null
PGPASSFILE='database/.pgpass' psql -w -h "${PGHOST:-localhost}" -p "${PGPORT:-5432}" -d postgres -U admin -c "create database upchieve;" 1> /dev/null
