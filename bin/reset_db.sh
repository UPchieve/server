...
PGPASSFILE='database/.pgpass' dropdb -h localhost -p 5432 -U admin -w --if-exists upchieve &> /dev/null
PGPASSFILE='database/.pgpass' psql -w -h localhost -p 5432 -d postgres -U admin -c "create database upchieve;" &> /dev/null
