import * as TrainingCtrl from '../../controllers/TrainingCtrl'
import * as TrainingCourseService from '../../services/TrainingCourseService'
import * as VolunteerService from '../../services/VolunteerService'
import { Router, Response } from 'express'
import { asString } from '../../utils/type-utils'
import { resError } from '../res-error'
import { extractUser } from '../extract-user'
import { Certifications } from '../../models/Volunteer'
import {
  userHasTakenQuiz,
  createQuizAction,
} from '../../models/UserAction/queries'
import { QUIZ_USER_ACTIONS } from '../../constants'
import { getQuizReviewMaterials } from '../../models/Question/queries'
import { getClient } from '../../db'
import type {
  QuestionsResponse,
  QuizScoreResponse,
  ReviewMaterialPublic,
  TrainingCourseResponse,
  UserTrainingCourseProgressUpdatePublic,
} from '../../contracts/training'
import {
  toQuestionPublic,
  toReviewMaterialPublic,
  toTrainingCourseWithUserProgressPublic,
  toUserTrainingCourseProgressUpdatePublic,
} from '../../public/training'
import { TrainingCourses } from '../../types/training'

export function routeTraining(router: Router): void {
  router.post(
    '/training/questions',
    async function (req, res: Response<QuestionsResponse>) {
      try {
        const questions = await TrainingCtrl.getQuestions(
          asString(req.body.category)
        )
        res.json({
          msg: 'Questions retrieved from database',
          questions: questions.map(toQuestionPublic),
        })
      } catch (err) {
        resError(res, err)
      }
    }
  )

  router.post(
    '/training/score',
    async function (req, res: Response<QuizScoreResponse>) {
      try {
        const { ip } = req
        const user = extractUser(req)

        const category = asString(req.body.category)
        const idAnswerMap = req.body.idAnswerMap // TODO: duck type validation

        const { tries, passed, score, idCorrectAnswerMap, isTrainingSubject } =
          await TrainingCtrl.getQuizScore({
            user: user,
            ip,
            category: category as keyof Certifications,
            idAnswerMap,
          })

        if (passed) {
          await createQuizAction(
            {
              userId: user.id,
              action: QUIZ_USER_ACTIONS.PASSED,
              quizSubcategory: category,
              ipAddress: ip,
            },
            getClient()
          ) // @TODO service fn
        } else {
          // we want to queue a job to send this email only if this is the first time
          // a volunteer has taken a quiz ever, and they failed it
          // must come before th quizActionCreator call or will never fire
          // because there would always be a failed quiz
          const takenQuizBefore = await userHasTakenQuiz(user.id)
          if (!takenQuizBefore)
            await VolunteerService.queueFailedFirstAttemptedQuizEmail(
              category,
              user.email,
              user.firstName,
              user.id
            )
          await createQuizAction(
            {
              userId: user.id,
              action: QUIZ_USER_ACTIONS.FAILED,
              quizSubcategory: category,
              ipAddress: ip,
            },
            getClient()
          ) // @TODO service fn
        }

        res.json({
          msg: 'Score calculated and saved',
          tries,
          passed,
          score,
          idCorrectAnswerMap,
          isTrainingSubject,
        })
      } catch (err) {
        resError(res, err)
      }
    }
  )

  router.get(
    '/training/review/:category',
    async function (req, res: Response<ReviewMaterialPublic[]>) {
      try {
        const user = extractUser(req)
        const category = asString(req.params.category)
        const { ip: ipAddress } = req

        await createQuizAction(
          {
            userId: user.id,
            action: QUIZ_USER_ACTIONS.VIEWED_MATERIALS,
            quizSubcategory: category,
            ipAddress: ipAddress,
          },
          getClient()
        ) // @TODO This needs to be wrapped in a service function that uses runInTransaction

        const resultList = await getQuizReviewMaterials(category)
        if (resultList) {
          res.status(200).json(resultList.map(toReviewMaterialPublic))
        }
      } catch (err) {
        resError(res, err)
      }
    }
  )

  router.get(
    '/training/course/:courseKey',
    async function (req, res: Response<TrainingCourseResponse>) {
      try {
        const user = extractUser(req)
        const courseKey = asString(req.params.courseKey)
        const course = await TrainingCourseService.getCourse(
          user,
          courseKey as keyof TrainingCourses
        )
        res
          .status(200)
          .json({ course: toTrainingCourseWithUserProgressPublic(course) })
      } catch (err) {
        resError(res, err)
      }
    }
  )

  router.post(
    '/training/course/:courseKey/progress',
    async function (
      req,
      res: Response<UserTrainingCourseProgressUpdatePublic>
    ) {
      try {
        const user = extractUser(req)
        const courseKey = asString(req.params.courseKey)
        const materialKey = asString(req.body.materialKey)
        const result = await TrainingCourseService.recordProgress(
          user,
          courseKey as keyof TrainingCourses,
          materialKey
        )
        res.status(200).json(toUserTrainingCourseProgressUpdatePublic(result))
      } catch (err) {
        resError(res, err)
      }
    }
  )
}
