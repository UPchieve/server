...
import {
  ...
  toSurveyQueryResponsePublic,
} from '../../public/surveys'

export function routeSurvey(router: expressWs.Router): void {
  router.post('/survey/save', async (req, res: Response<void>) => {
    try {
  ...
  router.get(
    '/survey/presession/:sessionId/goal',
    async (req, res: Response<PresessionGoalResponse>) => {
  ...
  router.get(
    '/survey/presession',
    async (req, res: Response<SurveyQueryResponsePublic>) => {
      try {
        ...
        res.json(toSurveyQueryResponsePublic(survey))
      } catch (error) {
        resError(res, error)
      }
    }
  ...
  router.get(
    ...
    async (req, res: Response<VolunteerContextResponsePublic>) => {
      try {
        const { sessionId } = req.params
  ...
  router.get(
    '/survey/postsession',
    async (req, res: Response<SurveyDefinitionResponse>) => {
      try {
        ...
        res.json({
          survey: survey ? toSurveyQueryResponsePublic(survey) : undefined,
        })
      } catch (error) {
        resError(res, error)
      }
  ...
  router.get(
    '/survey/postsession/response',
    async (req, res: Response<PostsessionSurveyResponsePublic[]>) => {
      try {
        const { sessionId, role } = req.query
        let parsedRole = parseUserRole(asString(role))
        ...
        res.json(surveyResponse.map(toPostsessionSurveyResponsePublic))
      } catch (error) {
        resError(res, error)
      }
    }
  ...
  router.get(
    ...
    async function (req, res: Response<SurveyDefinitionResponse>) {
      try {
  ...
  router.get(
    ...
    async function (req, res: Response<GetSimpleSurveyResponse>) {
      try {
        const user = extractUser(req)
        ...
        res.json({ survey: survey.map(toSimpleSurveyResponsePublic) })
      } catch (err) {
        resError(res, err)
      }
    }
  ...
  router.get(
    ...
    async (req, res: Response<SurveyQueryResponsePublic>) => {
      try {
        ...
        return res.json(toSurveyQueryResponsePublic(survey))
      } catch (err) {
        resError(res, err)
      }
    }
  ...
  router.get(
    '/survey/impact-study/responses',
    async (req, res: Response<SurveyQueryResponsePublic | undefined>) => {
      try {
        const user = extractUser(req)
        const survey = await getLatestImpactStudySurveyResponses(user.id)
        return res.json(
          survey ? toSurveyQueryResponsePublic(survey) : undefined
        )
      } catch (err) {
        resError(res, err)
      }
