...
import { toVolunteerToReviewPublic } from '../../public/volunteers'
import type {
  AggregateAvailabitiesResponse,
  LastHoursUpdatedResponse,
  VolunteerPresenceResponse,
  VolunteersToReviewResponse,
} from '../../contracts/volunteers'
...
export function routeVolunteers(router: Router): void {
  router.get(
    ...
    async function (req, res: Response<AggregateAvailabitiesResponse>) {
      try {
  ...
  router.get(
    ...
    async function (
      req,
      res: Response<VolunteersToReviewResponse | ErrorResponse>
    ) {
      try {
        ...
        res.json({
          volunteers: volunteers.map(toVolunteerToReviewPublic),
          isLastPage,
        })
      } catch (error) {
        res.status(500).json({
  ...
  router.post(
    ...
    authPassport.isAdmin,
    async function (req, res: Response<void | ErrorResponse>) {
      try {
  ...
  router.get(
    '/volunteers/hours-last-updated',
    async function (
      req,
      res: Response<LastHoursUpdatedResponse | void | ErrorResponse>
    ) {
      try {
  ...
  router.get(
    '/volunteers/presence',
    async function (req, res: Response<VolunteerPresenceResponse>) {
      try {
