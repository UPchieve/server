...
import { invokeChatApi, MODEL_ID as OPENAI_MODEL_ID } from './OpenAIService'
...
export async function generateSessionSummary(
  ...
  logger.info(
    `Session: ${metadata.sessionId} received session summary completion ${result} for userType ${metadata.userType} with response ${result}`
  )
  return response
}
