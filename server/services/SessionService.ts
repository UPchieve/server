...
export async function ensureCanJoinSession(
  ...
  const hasShadowBannedVolunteer = banStatuses.some(
    (status) =>
      status.banType === 'shadow' && isVolunteer && status.id === user.id
  )
  ...
  const hasShadowBannedStudent = banStatuses.some(
    (status) => status.id === session.studentId && status.banType === 'shadow'
  )
