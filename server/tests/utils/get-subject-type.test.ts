...
describe('Get subject type', () => {
  ...
  test('Science', () => {
    ...
    expect(getSubjectType(SCIENCE_SUBJECTS.BIOLOGY)).toEqual(SUBJECT_TYPES.SCIENCE)
  })
  test('SAT', () => {
  ...
  test('College', () => {
    expect(getSubjectType(COLLEGE_CERTS.APPLICATIONS)).toEqual(SUBJECT_TYPES.COLLEGE)
    expect(getSubjectType(COLLEGE_SUBJECTS.APPLICATIONS)).toEqual(SUBJECT_TYPES.COLLEGE)
  })
  test('Reading and writing', () => {
    expect(getSubjectType(READING_WRITING_CERTS.HUMANITIES_ESSAYS)).toEqual(SUBJECT_TYPES.READING_WRITING)
    expect(getSubjectType(READING_WRITING_SUBJECTS.HUMANITIES_ESSAYS)).toEqual(SUBJECT_TYPES.READING_WRITING)
  })
  test('Training', () => {
    expect(getSubjectType(TRAINING.UPCHIEVE_101)).toEqual(SUBJECT_TYPES.TRAINING)
  })
