...
describe('deidentifyUserJob', () => {
  ...
  test('deidentifies the users_student_partner_orgs_instances when the student is part of partner orgs', async () => {
    ...
    const schoolSpoId = (
      await client.query(
        `INSERT INTO upchieve.student_partner_orgs (id, name, key, school_id) VALUES (gen_random_uuid(), 'School Partner Org', 'school-partner', $1) RETURNING id`,
        [schoolId]
      )
    ).rows[0].id
    const spoId = (
      await client.query(
        `INSERT INTO upchieve.student_partner_orgs (id, name, key) VALUES (gen_random_uuid(), 'Partner Org', 'partner') RETURNING id`
      )
    ).rows[0].id
    ...
    const dummySchoolOrgId = (await client.query(`SELECT id FROM student_partner_orgs WHERE key = 'dummy-school-org'`)).rows[0].id
    const dummyOrgId = (await client.query(`SELECT id FROM student_partner_orgs WHERE key = 'dummy-org'`)).rows[0].id
    ...
    const orgInstancesAfter = await client.query(
      `
        SELECT * FROM upchieve.users_student_partner_orgs_instances WHERE student_partner_org_id IN ($1, $2)
      `,
      [schoolSpoId, spoId]
    )
  ...
  test('deidentifies the users_volunteer_partner_orgs_instances when the volunteer is part of a partner org', async () => {
    ...
    const vpoResults = await client.query(
      `INSERT INTO upchieve.volunteer_partner_orgs (id, name, key) VALUES (gen_random_uuid(), 'Volunteer Partner Org', 'volunteer-partner') RETURNING id`
    )
    const vpoId = vpoResults.rows[0].id
    ...
    const dummyVpoId = (await client.query(`SELECT id FROM volunteer_partner_orgs WHERE key = 'dummy-org'`)).rows[0].id
