...
export default async (job: Job<DeidentifyUserJob>): Promise<void> => {
  ...
  await runInTransaction(async (tc: TransactionClient) => {
    ...
    if (photoIdKeyResult.rows.length) {
      const photoIdKey = photoIdKeyResult.rows[0]
...
async function deidentifyRows(
  ...
  const spoInstancesResult = await tc.query(
    `SELECT * FROM users_student_partner_orgs_instances uspoi JOIN student_partner_orgs spo ON uspoi.student_partner_org_id = spo.id WHERE user_id = $1`,
    [userId]
  )
  if (spoInstancesResult.rows.length) {
    ...
    for (const instance of spoInstancesResult.rows) {
      ...
      await tc.query(
        `INSERT INTO users_student_partner_orgs_instances (student_partner_org_id, deactivated_on, created_at, updated_at)
       VALUES ($1, $2, $3, $3)`,
        [
