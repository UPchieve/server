...
const getStudentsPresessionGoalIR: any = {"name":"getStudentsPresessionGoal","params":[{"name":"sessionId","required":true,"transform":{"type":"scalar"},"codeRefs":{"used":[{"a":1821,"b":1830,"line":85,"col":30}]}}],"usedParamSet":{"sessionId":true},"statement":{"body":"SELECT\n  (\n    CASE\n      WHEN src.choice_text = 'Other' THEN uss.open_response\n      ELSE src.choice_text\n    END\n  ) AS goal\nFROM\n  users_surveys\n  JOIN users_surveys_submissions uss on users_surveys.id = uss.user_survey_id\n  JOIN survey_questions sq ON uss.survey_question_id = sq.id\n  JOIN survey_response_choices src ON uss.survey_response_choice_id = src.id\nWHERE\n  users_surveys.session_id = :sessionId!\n  AND sq.question_text = 'What is your primary goal for today''s session?'","loc":{"a":1421,"b":1905,"line":72,"col":0}}};
