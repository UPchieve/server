...
export async function getIpByRawString(rawIpString: string): Promise<IpAddress | undefined> {
  try {
    const ip = await IpAddressModel.findOne({ ip: cleanIpString(rawIpString) }).lean().exec()
...
export async function createIpByRawString(rawIpString: string): Promise<IpAddress> {
  try {
