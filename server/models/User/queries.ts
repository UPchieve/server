...
export async function getUserIdByPhone(phone: string): Promise<Types.ObjectId | undefined> {
  try {
...
export async function getUserIdByEmail(email: string): Promise<Types.ObjectId | undefined> {
  try {
...
export async function getUserIdByReferralCode(referralCode: string): Promise<Types.ObjectId | undefined> {
  try {
...
export async function addUserPastSessionById(userId: Types.ObjectId, sessionId: Types.ObjectId): Promise<void> {
  try {
    const result = await UserModel.updateOne(
      { _id: userId },
...
export async function updateUserLastActivityById(userId: Types.ObjectId, lastActivityAt: Date) {
  try {
    const result = await UserModel.updateOne({ _id: userId }, { lastActivityAt })
    if (!result.ok) throw new RepoUpdateError('Update query did not return "ok"')
  } catch (err) {
    if (err instanceof RepoUpdateError) throw err
    throw new RepoUpdateError(err)
...
export async function banUserById(userId: Types.ObjectId, banReason: USER_BAN_REASON) {
  try {
    const result = await UserModel.updateOne(
      { _id: userId },
