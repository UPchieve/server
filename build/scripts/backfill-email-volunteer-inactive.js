...
async function BackfillEmailVolunteerInactive(job) {
    ...
    if (todaysDate >= blackoutPeriodStart && todaysDate <= blackoutPeriodEnd) {
        console.log(`Skipping ${jobs_1.Jobs.EmailVolunteerInactive} because today's date, ${new Date(todaysDate).toISOString()}, is within the blackout period: ${new Date(blackoutPeriodStart).toISOString()} - ${new Date(blackoutPeriodEnd).toISOString()}`);
        return;
    }
    ...
    if (volunteers) {
        ...
        const errors = [];
        try {
            await sendEmailToInactiveVolunteers(inactiveThirtyDays, jobs_1.Jobs.EmailVolunteerInactiveThirtyDays, MailService.sendVolunteerInactiveThirtyDays, InactiveGroup.inactiveThirtyDays);
        }
        catch (error) {
        ...
        try {
            await sendEmailToInactiveVolunteers(inactiveSixtyDays, jobs_1.Jobs.EmailVolunteerInactiveSixtyDays, MailService.sendVolunteerInactiveSixtyDays, InactiveGroup.inactiveSixtyDays);
        }
        catch (error) {
            if (Array.isArray(error))
                errors.push(...error);
        ...
        try {
            await sendEmailToInactiveVolunteers(inactiveNinetyDays, jobs_1.Jobs.EmailVolunteerInactiveNinetyDays, MailService.sendVolunteerInactiveNinetyDays, InactiveGroup.inactiveNinetyDays);
        }
        catch (error) {
            if (Array.isArray(error))
                errors.push(...error);
...
exports.default = BackfillEmailVolunteerInactive;
var InactiveGroup;
...
function getStartOfDayFromDaysAgo(start, daysAgo) {
    return (0, moment_1.default)(start)
        .utc()
        .subtract(daysAgo, 'days')
        .startOf('day')
        .toDate();
}
function getEndOfDayFromDaysAgo(start, daysAgo) {
    return (0, moment_1.default)(start)
        .utc()
        .subtract(daysAgo, 'days')
        .endOf('day')
