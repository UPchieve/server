...
const bannedServiceProviderList = process.env.SUBWAY_BANNED_SERVICE_PROVIDERS || 'Example';
...
const config = {
    ...
    sessionCookieMaxAge: parseInt(process.env.SUBWAY_SESSION_COOKIE_MAX_AGE || '5184000000'),
    saltRounds: 10,
    smtp: {
        host: process.env.SUBWAY_SMTP_HOST || 'smtp.mailtrap.io',
        port: parseInt(process.env.SUBWAY_SMTP_PORT || '2525'),
        secure: process.env.SUBWAY_SMTP_SECURE || false,
    ...
    sendgrid: {
        ...
        customWeeklyHourSummaryIntroEmailTemplate: 'd-ada36745cc724549833b867f514435a6',
        ...
        partnerVolunteerLowHoursSelectedTemplate: 'd-476522cdd78e4c4ebc2af51a2086a640',
        ...
        contactList: {
            students: process.env.SUBWAY_STUDENT_CONTACT_LIST ||
                '1111111a-111b-111c-111d-11111111111e',
            volunteers: process.env.SUBWAY_VOLUNTEER_CONTACT_LIST ||
                '1111111a-111b-111c-111d-11111111111e',
        },
    },
    ...
    mail: {
        senders: {
            ...
            recruitment: process.env.SUBWAY_RECRUITMENT_EMAIL_SENDER || 'example@example.org',
            students: process.env.SUBWAY_STUDENTS_EMAIL_SENDER || 'example@example.org',
            volunteerManager: process.env.SUBWAY_VOLUNTEER_MANAGER_EMAIL_SENDER ||
                'example@example.org',
            studentOutreachManager: process.env.SUBWAY_STUDENT_OUTREACH_MANAGER_EMAIL_SENDER ||
                'example@example.org',
            corporatePartnershipsManager: process.env.SUBWAY_CORPORATE_PARTNERSHIPS_MANAGER_EMAIL_SENDER ||
                'example@example.org',
            crisis: process.env.SUBWAY_CRISIS_EMAIL_SENDER || 'example@example.org',
        },
        receivers: {
            contact: process.env.SUBWAY_CONTACT_EMAIL_RECEIVER || 'example@example.org',
            staff: process.env.SUBWAY_STAFF_EMAIL_RECEIVER || 'example@example.org',
            support: process.env.SUBWAY_SUPPORT_EMAIL_RECEIVER || 'example@example.org',
            recruitment: process.env.SUBWAY_RECRUITMENT_EMAIL_RECEIVER || 'example@example.org',
            students: process.env.SUBWAY_STUDENTS_EMAIL_RECEIVER || 'example@example.org',
            volunteerManager: process.env.SUBWAY_VOLUNTEER_MANAGER_EMAIL_RECEIVER ||
                'example@example.org',
            studentOutreachManager: process.env.SUBWAY_STUDENT_OUTREACH_MANAGER_EMAIL_RECEIVER ||
                'example@example.org',
            corporatePartnershipsManager: process.env.SUBWAY_CORPORATE_PARTNERSHIPS_MANAGER_EMAIL_RECEIVER ||
                'example@example.org',
        },
        people: {
            volunteerManager: {
            ...
            corporatePartnershipsManager: {
                firstName: process.env.SUBWAY_CORPORATE_PARTNERSHIPS_MANAGER_FIRST_NAME || '',
                lastName: process.env.SUBWAY_CORPORATE_PARTNERSHIPS_MANAGER_LAST_NAME || '',
            },
        },
    },
    client: {
    ...
    customVolunteerPartnerOrgs: (process.env.SUBWAY_CUSTOM_VOLUNTEER_PARTNER_ORGS || 'big-telecom').split(','),
    priorityMatchingPartnerOrgs: (process.env.SUBWAY_PRIORITY_MATCHING_PARTNER_ORGS || 'bogus').split(','),
    priorityMatchingSponsorOrgs: (process.env.SUBWAY_PRIORITY_MATCHING_SPONSOR_ORGS || 'onlySponsorsSchools').split(','),
    corporatePartnerReports: {
        customAnalyticsReportPartnerOrgs: (process.env.SUBWAY_CUSTOM_ANALYTICS_PARTNER_ORGS || 'big-telecom').split(','),
        batchSize: Number(process.env.SUBWAY_CORPORATE_PARTNER_REPORTS_BATCH_SIZE) || 100,
    },
    clusterServerAddress: process.env.SUBWAY_CLUSTER_SERVER_ADDRESS || 'localhost',
    assistmentsBaseURL: process.env.SUBWAY_ASSISTMENTS_BASE_URL || 'https://example.com',
    assistmentsToken: process.env.SUBWAY_ASSISTMENTS_TOKEN || 'bogus',
    assistmentsAuthSchema: process.env.SUBWAY_ASSISTMENTS_AUTH_SCHEMA || 'token={TOKEN}',
    cacheKeys: {
    ...
    twilioAccountVerificationServiceSid: process.env.SUBWAY_TWILIO_ACCOUNT_VERIFICATION_SERVICE_SID || '',
    twilioVerificationRateLimitUniqueName: 'userId',
    twilioVerificationRateLimitMaxRetries: Number(process.env.SUBWAY_TWILIO_VERIFICATION_RATELIMIT_MAX_RETRIES) || 4,
    twilioVerificationRateLimitIntervalSeconds: Number(process.env.SUBWAY_TWILIO_VERIFICATION_RATELIMIT_INTERVAL_SECONDS) ||
        60,
    ...
    postgresAnalyticsHost: process.env.SUBWAY_POSTGRES_ANALYTICS_HOST || 'localhost',
    ...
    awsS3: {
        ...
        secretAccessKey: process.env.SUBWAY_SECRET_ACCESS_KEY || 'SECRETACCESSKEY789',
        region: process.env.SUBWAY_AWS_REGION || 'us-east-2',
        photoIdBucket: process.env.SUBWAY_PHOTO_ID_BUCKET || 'photo-id-bucket',
        sessionPhotoBucket: process.env.SUBWAY_SESSION_PHOTO_BUCKET || 'session-photo-bucket',
    },
    posthogToken: process.env.SUBWAY_POSTHOG_TOKEN || 'bogus',
    posthogPersonalApiToken: process.env.SUBWAY_POSTHOG_PERSONAL_API_TOKEN || 'bogus',
    /**
     *
    ...
    whiteboardStorageSubscriptionId: process.env.SUBWAY_AZURE_WHITEBOARD_SUBSCRIPTION_ID || 'bogus',
    whiteboardStorageAccountName: process.env.SUBWAY_WHITEBOARD_STORAGE_ACCOUNT_NAME || 'bogus',
    whiteboardStorageContainer: process.env.SUBWAY_WHITEBOARD_STORAGE_CONTAINER || 'bogus',
    ...
    favoriteVolunteerLimit: Number(process.env.SUBWAY_FAVORITE_VOLUNTEER_LIMIT) || 20,
    eligibleIncomeThreshold: Number(process.env.SUBWAY_ELIGIBLE_INCOME_THRESHOLD) || 60000,
    eligibleFRLThreshold: Number(process.env.SUBWAY_ELIGIBLE_FRL_THRESHOLD) || 0.4,
    customManualStudentPartnerOrg: process.env.SUBWAY_CUSTOM_MANUAL_STUDENT_PARTNER_ORG || 'college-mentors',
    ...
    googleRecaptchaThreshold: isNaN(Number(process.env.GOOGLE_RECAPTCHA_THRESHOLD))
        ? 0.5
