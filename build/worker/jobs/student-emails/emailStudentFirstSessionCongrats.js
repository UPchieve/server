...
exports.default = async (job) => {
    ...
    const student = await (0, Session_1.getStudentForEmailFirstSession)(sessionId);
    if (student) {
        const { id: studentId, firstName, email } = student;
        try {
            await MailService.sendStudentFirstSessionCongrats(email, firstName);
            (0, logger_1.log)(`Sent ${currentJob} to student ${studentId}`);
        }
        catch (error) {
            throw new Error(`Failed to send ${currentJob} to student ${studentId}: ${error}`);
        }
    }
};
