...
4. (optional) Run `npm run dev:worker` to start the worker process. The dev worker will automatically attempt to connect to your local Redis instance and read jobs from there.
...
To schedule cron jobs, enqueue the UpdateCronJobs job from `server/scripts/testing-jobs.ts`
